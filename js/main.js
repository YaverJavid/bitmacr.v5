...
function exportImage (){
    ...
    if (cellBorderWidthSlider.value == 0) {
        for (let i = 0; i < paintCells.length; i++) {
            let currentCellColor = buffer.getItem()[i]
        ...
        for (let i = 0; i < paintCells.length; i++) {
            ...
            if (currentCellColor != "rgba(0, 0, 0, 0)")
                drawRectangle(ctx, currentX, currentY, cellWidth, cellWidth, cellBorderColorSelector.value, currentCellColor, cellBorderWidthSlider.value)
            currentX += cellWidth
            if (Math.round(currentX) == Math.round(canvas.width)) {
                currentX = 0
                currentY += cellWidth
